@php
    ...
    $sidebarMenuLists = [
        ...
        'Article Tab' => [
            ['url' => 'backend/post-category', 'icon' => 'bx-sm bx bx-category', 'label' => 'Category', 'permission' => 'view_post_categories', 'new_tab' => false, 'exact' => false],
                ...
                @foreach($sidebarMenuLists as $key => $items)
                    @if(is_int($key))
                        @can($items['permission'] ?? '')
                            <li class="sidebar-item{{ isActive($items['url'], $items['exact'] ?? false) ? ' active' : '' }}">
                            ...
                            @foreach($items as $item)
                                @if(isset($item['submenu']))
                                    @if(collect($item['submenu'])->contains(fn($submenuItem) => auth()->user()->can($submenuItem['permission'] ?? '')))
                                        <li class="sidebar-item has-sub {{ collect($item['submenu'])->contains(fn($submenuItem) => isActive($submenuItem['url'], $submenuItem['exact'] ?? false)) ? ' active' : '' }}">
                                            <a href="#" class='sidebar-link'>
                                                <i class="{{ $item['icon'] }}"></i>
                                                <span>{{ $item['label'] }}</span>
                                            </a>
                                            <ul class="submenu {{ collect($item['submenu'])->contains(fn($submenuItem) => isActive($submenuItem['url'], $submenuItem['exact'] ?? false)) ? 'active' : '' }}">
                                                @foreach($item['submenu'] as $submenuItem)
                                                    @can($submenuItem['permission'] ?? '')
                                                        <li class="submenu-item {{ isActive($submenuItem['url'], $submenuItem['exact'] ?? false) ? 'active' : '' }}">
                                ...
                                @else
                                    @can($item['permission'] ?? '')
                                        <li class="sidebar-item{{ isActive($item['url'], $item['exact'] ?? false) ? ' active' : '' }}">
